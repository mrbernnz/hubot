...
#
# Commands:
#   welcome <new welcome message> - Update the welcome message
#
# Author:
#   jjasghar
...
module.exports = (robot) ->
  ...
  robot.enter (msg) ->
    ...
    if users in stored_users # if the user above is in the stored_users do nothing
      ...
      msg.send "Welcome, #{msg.message.user.name}, #{welcome}, #{msg.message.user.name}" # if it's the first time you're seeing them give them the welcome message
